export class Conductor {
  constructor() {
    ...
    this.surtidores = guardados ? JSON.parse(guardados) : [
      { 
        nombre: 'Surtidor A',
  ...
  editarSurtidor(nombreOriginal, ...args) {
    const surtidor = this.surtidores.find(x => x.nombre === nombreOriginal);
    if (!surtidor) return;

    let [nuevoNombre, nuevaDireccion, nuevoEstado, nuevaFila, nuevaZona, 
         nuevosLitros, nuevaApertura, nuevoCierre, nuevoContacto] = [];

    if (args.length === 4) {
      [nuevoNombre, nuevoEstado, nuevaFila, nuevaZona] = args;
      nuevaDireccion = surtidor.direccion;
      nuevosLitros = surtidor.litros;
      nuevaApertura = surtidor.apertura;
      nuevoCierre = surtidor.cierre;
      nuevoContacto = surtidor.contacto;
    }
    else if (args.length === 8) {
      [nuevoNombre, nuevoEstado, nuevaFila, nuevaZona, 
       nuevosLitros, nuevaApertura, nuevoCierre, nuevoContacto] = args;
      nuevaDireccion = surtidor.direccion;
    }
    else if (args.length === 9) {
      [nuevoNombre, nuevaDireccion, nuevoEstado, nuevaFila, nuevaZona, 
       nuevosLitros, nuevaApertura, nuevoCierre, nuevoContacto] = args;
    } else {
      throw new Error('editarSurtidor: parámetros inválidos');
    }

    Object.assign(surtidor, {
      nombre: nuevoNombre,
      direccion: nuevaDireccion,
      estado: String(nuevoEstado).trim(),
      fila: parseInt(nuevaFila, 10),
      zona: nuevaZona,
      litros: parseInt(nuevosLitros, 10),
  ...
  generarTicket(estacion, surtidor, tipoCombustible) {
    if (!surtidor || typeof surtidor !== 'object' || 
        !surtidor.nombre || surtidor.litros == null) {
